...
function ThreatGauge({ darkMode, score = 72 }) {
  const radius = 80;
  const stroke = 14;
  const normalizedRadius = radius - stroke / 2;
  const circumference = normalizedRadius * Math.PI;
  const progress = (score / 100) * circumference;
  const color = score > 70 ? DANGER : score > 40 ? WARN : "#10b981";
  const label = score > 70 ? "HIGH THREAT" : score > 40 ? "MODERATE" : "LOW RISK";
  ...
  return (
    <div style={{ background: s.card, border: `1.5px solid ${s.border}`, borderRadius: "18px", padding: "1.75rem", backdropFilter: "blur(12px)", textAlign: "center", position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "3px", background: `linear-gradient(90deg, ${color}, ${color}66)` }} />
      <p style={{ fontSize: "12px", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: s.muted, marginBottom: "20px" }}>Competitor Threat Level</p>
      <div style={{ display: "flex", justifyContent: "center" }}>
        <svg width={radius * 2} height={radius + stroke + 10} style={{ overflow: "visible" }}>
          <path d={`M ${stroke / 2} ${radius} A ${normalizedRadius} ${normalizedRadius} 0 0 1 ${radius * 2 - stroke / 2} ${radius}`}
            fill="none" stroke={darkMode ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.07)"} strokeWidth={stroke} strokeLinecap="round" />
          <path d={`M ${stroke / 2} ${radius} A ${normalizedRadius} ${normalizedRadius} 0 0 1 ${radius * 2 - stroke / 2} ${radius}`}
            fill="none" stroke={color} strokeWidth={stroke} strokeLinecap="round"
            strokeDasharray={`${progress} ${circumference}`}
            style={{ filter: `drop-shadow(0 0 8px ${color}99)` }} />
          <text x={radius} y={radius - 10} textAnchor="middle" fill={s.text} fontSize="36" fontWeight="800" fontFamily="inherit">{score}</text>
          <text x={radius} y={radius + 16} textAnchor="middle" fill={color} fontSize="11" fontWeight="800" fontFamily="inherit" letterSpacing="1.5">{label}</text>
        </svg>
      </div>
      <p style={{ fontSize: "13px", color: s.muted, marginTop: "14px", lineHeight: "1.6" }}>
        Based on pricing, feature & messaging signals
      </p>
      <div style={{ display: "flex", justifyContent: "center", gap: "16px", marginTop: "14px" }}>
        {[["Low", "#10b981"], ["Med", WARN], ["High", DANGER]].map(([l, c]) => (
...
function SEOChart({ darkMode }) {
  const data = [
    { label: "Domain Authority", you: 41, them: 58, max: 100, color: ACCENT },
    { label: "Backlinks (K)", you: 18, them: 42, max: 60, color: TEAL },
    { label: "Content Score", you: 52, them: 74, max: 100, color: "#a78bfa" },
    { label: "Keyword Coverage", you: 38, them: 67, max: 100, color: WARN },
  ];
  ...
  const s = {
    ...
    text: darkMode ? "#f0eeff" : "#1a1825",
    muted: darkMode ? "#c0bdd8" : "#4a4860",
    track: darkMode ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.06)",
  };

  return (
    <div style={{ background: s.card, border: `1.5px solid ${s.border}`, borderRadius: "18px", padding: "1.75rem", backdropFilter: "blur(12px)", position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "3px", background: "linear-gradient(135deg, #7c6ff7, #14b8a6)" }} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "20px" }}>
        <p style={{ fontSize: "12px", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: s.muted }}>SEO Authority Gap</p>
        <div style={{ display: "flex", gap: "14px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            <div style={{ width: "10px", height: "3px", borderRadius: "2px", background: ACCENT }} />
            <span style={{ fontSize: "11px", color: s.muted, fontWeight: 600 }}>You</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            <div style={{ width: "10px", height: "3px", borderRadius: "2px", background: darkMode ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.2)" }} />
            <span style={{ fontSize: "11px", color: s.muted, fontWeight: 600 }}>Competitor</span>
          </div>
        </div>
      </div>
      {data.map((d, i) => (
        <div key={i} style={{ marginBottom: i < data.length - 1 ? "18px" : "0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "7px" }}>
            <span style={{ fontSize: "13px", color: s.muted, fontWeight: 600 }}>{d.label}</span>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <span style={{ fontSize: "14px", fontWeight: 800, color: d.color }}>{d.you}</span>
              <span style={{ fontSize: "11px", color: s.muted }}>vs</span>
              <span style={{ fontSize: "14px", fontWeight: 700, color: s.text }}>{d.them}</span>
            </div>
          </div>
          <div style={{ position: "relative", height: "24px" }}>
            {/* Competitor bar (background) */}
            <div style={{ position: "absolute", top: "8px", left: 0, right: 0, height: "8px", background: s.track, borderRadius: "8px", overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${(d.them / d.max) * 100}%`, background: darkMode ? "rgba(255,255,255,0.18)" : "rgba(0,0,0,0.15)", borderRadius: "8px" }} />
            </div>
            {/* Your bar (foreground) */}
            <div style={{ position: "absolute", top: "8px", left: 0, height: "8px", width: `${(d.you / d.max) * 100}%`, background: `linear-gradient(90deg, ${d.color}, ${d.color}cc)`, borderRadius: "8px", boxShadow: `0 0 10px ${d.color}55`, zIndex: 1 }} />
          </div>
        </div>
      ))}
    </div>
...
function ActivityTimeline({ darkMode }) {
  const events = [
    { date: "3 days ago", label: "Pricing page restructured", detail: "Pro plan reduced, free tier added", type: "danger", icon: "💰" },
    { date: "1 week ago", label: "New integration launched", detail: "Slack-native CRM shipped to all plans", type: "warning", icon: "🚀" },
    { date: "2 weeks ago", label: "Content strategy shift", detail: "AI churn reduction case study published", type: "warning", icon: "📝" },
    { date: "1 month ago", label: "Homepage messaging update", detail: "Moved to outcome-led hero copy", type: "info", icon: "🎯" },
  ];

  const typeColor = { danger: DANGER, warning: WARN, info: ACCENT };
  ...
  const s = {
    ...
    text: darkMode ? "#f0eeff" : "#1a1825",
    muted: darkMode ? "#c0bdd8" : "#4a4860",
    sub: darkMode ? "#8a88a0" : "#9896a8",
    line: darkMode ? "rgba(124,111,247,0.2)" : "rgba(124,111,247,0.15)",
  };

  return (
    <div style={{ background: s.card, border: `1.5px solid ${s.border}`, borderRadius: "18px", padding: "1.75rem", backdropFilter: "blur(12px)", position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "3px", background: "linear-gradient(135deg, #a78bfa, #7c6ff7)" }} />
      <p style={{ fontSize: "12px", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: s.muted, marginBottom: "22px" }}>Competitor Activity Timeline</p>
      <div style={{ position: "relative" }}>
        <div style={{ position: "absolute", left: "17px", top: "8px", bottom: "8px", width: "1px", background: s.line }} />
        {events.map((e, i) => (
          <div key={i} style={{ display: "flex", gap: "16px", marginBottom: i < events.length - 1 ? "22px" : "0", alignItems: "flex-start" }}>
            <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: `${typeColor[e.type]}18`, border: `2px solid ${typeColor[e.type]}55`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", flexShrink: 0, position: "relative", zIndex: 1, backdropFilter: "blur(8px)" }}>
              {e.icon}
            </div>
            <div style={{ paddingTop: "4px", flex: 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "3px" }}>
                <p style={{ fontSize: "15px", fontWeight: 700, color: s.text, lineHeight: "1.3" }}>{e.label}</p>
                <span style={{ fontSize: "11px", color: s.sub, whiteSpace: "nowrap", marginLeft: "8px", marginTop: "2px" }}>{e.date}</span>
              </div>
              <p style={{ fontSize: "13px", color: s.muted, lineHeight: "1.4" }}>{e.detail}</p>
            </div>
          </div>
        ))}
      </div>
...
function ScoreRing({ label, score, color, darkMode }) {
  const size = 64;
  const stroke = 6;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "8px" }}>
      <div style={{ position: "relative", width: size, height: size }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "15px", fontWeight: 800, color: s.text }}>{score}</div>
      </div>
      <p style={{ fontSize: "12px", color: s.muted, textAlign: "center", fontWeight: 600, lineHeight: "1.3" }}>{label}</p>
    </div>
  );
}
...
const FALLBACK = {
  brief: (compName) => `${compName} has made aggressive moves this cycle — analysis of their website reveals key strategic shifts in pricing, messaging, and product positioning that directly impact your competitive standing. Their content strategy signals a deliberate pivot toward capturing your core buyer segment. The threat level is elevated and the window to respond with targeted counter-messaging is roughly 30–60 days before their campaign gains meaningful traction.`,
  audits: {
    ...
    messaging: "The competitor's homepage and content strategy reveal a deliberate pivot toward outcome-focused, ROI-led messaging — a calculated move to own the retention and revenue narrative. This angle resonates strongly with CFOs and economic buyers. Counter-position by publishing a customer ROI case study and testing outcome-led copy on your homepage hero section.",
  },
  strategy: {
    ...
    suggestions: [
      { t: "Launch a freemium starter tier", d: "Introduce a low-friction entry plan to compete directly on acquisition. Gate advanced features behind paid tiers to drive upgrades. This removes the #1 pricing objection in competitive deals." },
      { t: "Reframe messaging around outcomes", d: "Shift your homepage hero from feature descriptions to business outcome statements. Use customer proof points with real numbers to directly counter the competitor's ROI-led narrative." },
      { t: "Build SEO authority on uncontested keywords", d: "Identify high-intent long-tail terms the competitor doesn't rank for yet. Publish 3 cornerstone articles targeting these gaps in the next 60 days to capture organic traffic." },
    ],
  },
  seo: [
    { ico: "🎯", t: "Target high-intent comparison keywords", d: "Publish '[Your Brand] vs [Competitor]' pages targeting buyer-stage searches. These convert 3–5x higher than category pages and intercept competitor-branded traffic at the decision stage." },
    { ico: "🔗", t: "Close the domain authority gap aggressively", d: "Launch targeted outreach for backlinks from SaaS review sites and tech directories. Priority: G2, Capterra, and relevant marketplace listings in your category." },
    { ico: "📝", t: "Content gaps represent quick organic wins", d: "Competitor analysis reveals underserved topic clusters with strong buyer intent and low competition. Publishing 3-5 cornerstone articles could capture significant organic traffic within 90 days." },
  ],
  opps: [
    { cls: "c1", ctx: "POSITIONING OPPORTUNITY", t: "Move upmarket before your competitor locks in the SMB segment", d: "The competitor's pricing strategy signals a volume-over-value land-grab in the SMB segment. This is your opening to reframe your narrative around outcomes, ROI, and strategic value rather than feature lists. The mid-market and enterprise-adjacent buyer segment is growing faster and commands significantly higher LTV. Update your homepage to lead with revenue outcomes, develop a dedicated use-case landing page with industry-specific case studies, and launch a thought leadership report to capture top-of-funnel authority. This positions you as a strategic partner rather than a feature-for-feature competitor, insulating you from the price war entirely while attracting buyers who prioritize quality and long-term partnership over short-term cost savings." },
    { cls: "c2", ctx: "ACQUISITION OPPORTUNITY", t: "Run a targeted migration campaign while competitor customers reassess", d: "Strategic changes at a competitor create natural churn risk in their existing customer base. Customers who feel the value equation has shifted are actively researching alternatives, often within 60–90 days of the change. This is your window for a targeted competitive displacement campaign. Build a migration landing page with a competitive switch offer including free migration assistance and a price-lock guarantee. Run LinkedIn ads targeting relevant job titles at companies using competitor products. Create a pointed comparison page optimized for competitor-branded searches. These warm prospects are actively seeking alternatives and convert at significantly higher rates than cold traffic from broader awareness campaigns." },
    { cls: "c3", ctx: "MESSAGING OPPORTUNITY", t: "Own 'proof over promise' to win trust-driven buyers", d: "Your competitor is investing heavily in AI and automation messaging — powerful claims that attract attention but also generate skepticism among battle-hardened B2B buyers burned by technology overpromises. Industry research consistently shows that over 60% of B2B software buyers are skeptical of AI feature claims and want verified proof before committing budget. Your opportunity is to position as the trustworthy, results-proven alternative by leading with verified customer outcomes — real numbers, real logos, real case studies — rather than technology feature lists. Create a Proof Over Promise content series showcasing detailed customer success stories with measurable revenue and pipeline outcomes. This differentiates you on trust rather than features, a far more durable competitive advantage." },
  ],
};
...
export default function Dashboard({ scanData, onNewScan, onLogout, currentUser, darkMode, toggleDark }) {
  ...
  async function loadBrief() {
    try {
      const result = await callClaude(
        "You are a senior competitive intelligence analyst. Write sharp, executive-level briefs. Be direct and strategic.",
        `Write a 3-4 sentence executive intelligence brief for a CMO. Cover what is notable about the competitor, why it matters, and urgency level.\n\n${buildContext()}\n\nClient: ${clientUrl}\nCompetitor: ${compName} (${competitorUrl})`
      );
      setAiBrief(result);
  ...
  async function loadAuditDetail(id) {
    ...
    const prompts = {
      pricing: `Write a pricing intelligence brief (60-80 words). Cover: pricing gap, sales impact, one immediate action.\n\n${buildContext()}`,
      feature: `Write a product intelligence brief (60-80 words). Cover: feature gaps, target segment, competitive risk, recommended response.\n\n${buildContext()}`,
      messaging: `Write a messaging intelligence brief (60-80 words). Cover: competitor messaging strategy, why effective, how to counter-position.\n\n${buildContext()}`,
    };
    try {
      const result = await callClaude("You are a competitive intelligence analyst. Write actionable 60-80 word briefs.", prompts[id]);
  ...
  async function loadStrategy() {
    try {
      const result = await callClaude(
        "CMO-level strategist. Respond ONLY with valid JSON, no markdown.",
        `Gap analysis JSON:\n{"competitorDoing":["i1","i2","i3","i4"],"clientMissing":["i1","i2","i3","i4"],"suggestions":[{"t":"title","d":"2-3 sentence rec"},{"t":"title","d":"rec"},{"t":"title","d":"rec"}]}\n\n${buildContext()}`
      );
  ...
  async function loadSeo() {
    try {
      const result = await callClaude(
        "Expert SEO strategist. Respond ONLY with valid JSON, no markdown.",
        `3 SEO recs JSON:\n[{"ico":"🎯","t":"title","d":"25-35 word rec"},{"ico":"🔗","t":"title","d":"rec"},{"ico":"📝","t":"title","d":"rec"}]\n\n${buildContext()}`
      );
  ...
  async function loadOpps() {
    try {
      const result = await callClaude(
        "Marketing Director. Write strategic opportunity alerts 150+ words each. Respond ONLY with valid JSON, no markdown.",
  ...
  <div style={{ position: "fixed", top: "5%", right: "5%", width: "400px", height: "400px", zIndex: 0, borderRadius: "50%", background: "radial-gradient(circle, rgba(236,72,153,0.35) 0%, rgba(167,139,250,0.2) 40%, transparent 70%)", animation: "orb2 11s ease-in-out infinite reverse", pointerEvents: "none", filter: "blur(50px)" }} />
      <style>{`
        ...
        .audit-card:hover{border-color:rgba(124,111,247,0.45) !important;transform:translateY(-2px);box-shadow:0 8px 32px rgba(124,111,247,0.15) !important;}
      ...
      <div style={{ maxWidth: "980px", margin: "0 auto", padding: "28px 24px" }}>
        ...
        <div style={{ background: s.card, border: `1.5px solid rgba(124,111,247,0.3)`, borderRadius: "20px", padding: "2rem", marginBottom: "8px", position: "relative", overflow: "hidden", backdropFilter: "blur(12px)", boxShadow: "0 4px 40px rgba(124,111,247,0.12)" }}>
          ...
          <div style={{ position: "relative" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: "7px", fontSize: "11px", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: ACCENT, background: "rgba(124,111,247,0.12)", padding: "5px 14px", borderRadius: "20px", marginBottom: "16px", border: "1px solid rgba(124,111,247,0.25)" }}>
        ...
        {/* ── CHARTS ROW ── */}
        {sectionHead("Competitive intelligence overview")}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.6fr 1.4fr", gap: "14px", marginBottom: "8px" }}>
          <ThreatGauge darkMode={darkMode} score={72} />
          <SEOChart darkMode={darkMode} />
          <ActivityTimeline darkMode={darkMode} />
        </div>

        {/* Opportunity Score Rings */}
        <div style={{ ...cardStyle, marginTop: "14px" }}>
          <p style={{ fontSize: "12px", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: s.hint, marginBottom: "20px" }}>Opportunity Scores</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "16px" }}>
            <ScoreRing label="Pricing Gap" score={78} color={DANGER} darkMode={darkMode} />
            <ScoreRing label="Feature Gap" score={64} color={WARN} darkMode={darkMode} />
            <ScoreRing label="SEO Gap" score={55} color={ACCENT} darkMode={darkMode} />
            <ScoreRing label="Messaging Gap" score={70} color="#a78bfa" darkMode={darkMode} />
            <ScoreRing label="Overall Risk" score={72} color={TEAL} darkMode={darkMode} />
          </div>
        </div>
        ...
        {AUDITS.map((a) => (
          <div key={a.id} className="audit-card" onClick={() => toggleAudit(a.id)}
            style={{ ...cardStyle, cursor: "pointer", borderColor: expandedAudit === a.id ? "rgba(124,111,247,0.5)" : s.border }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: "16px" }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px", flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.8px", padding: "3px 10px", borderRadius: "6px", background: `${a.badgeCol}22`, color: a.badgeCol, border: `1px solid ${a.badgeCol}44` }}>{a.badge}</span>
                </div>
                <p style={{ fontSize: "15px", color: s.muted, lineHeight: "1.65" }}>{a.sub}</p>
              </div>
              <span style={{ color: s.muted, fontSize: "18px", transform: expandedAudit === a.id ? "rotate(180deg)" : "none", transition: "transform 0.2s", flexShrink: 0 }}>▾</span>
            </div>
            {expandedAudit === a.id && (
        ...
        {opps ? opps.map((o, i) => (
          <div key={i} style={{ background: s.card, border: `1.5px solid ${s.border}`, borderRadius: "18px", padding: "2rem", marginBottom: "14px", backdropFilter: "blur(12px)", boxShadow: s.glow, position: "relative", overflow: "hidden" }} className="fade-in">
            <div style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: "4px", background: oppGrads[o.cls] || GRAD2 }} />
            ...
            <div style={{ position: "relative" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
                <span style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: oppColors[o.cls] || ACCENT, background: `${oppColors[o.cls] || ACCENT}18`, padding: "4px 12px", borderRadius: "20px", border: `1px solid ${oppColors[o.cls] || ACCENT}30` }}>{o.ctx}</span>
                <span style={{ fontSize: "12px", color: s.hint, fontWeight: 600 }}>Opportunity {i + 1}</span>
              </div>
